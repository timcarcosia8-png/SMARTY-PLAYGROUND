...
// Check verification and avatar status
$check = $conn->prepare("SELECT avatar, is_verified FROM users WHERE user_id = ?");
...
// If user already verified and has avatar → dashboard
if ($user && $user['is_verified'] == 1 && !empty($user['avatar'])) {

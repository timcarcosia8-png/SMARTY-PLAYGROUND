...
<head>
    ...
    <title>Lesson 4: Let Us Try - Medial Short /a/ Sound</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        .game-container {
            ...
            background: linear-gradient(to bottom, #6b2fb5, #2d1b4e);
            padding: 20px;
        }
        ...
        .button-green {
            background: linear-gradient(to right, #86efac, #5eead4);
            color: #1f2937;
...
<body>
    ...
    <script>
        ...
        async function loadQuestions() {
            try {
                ...
                if (questions.length > 0) {
                    ...
                    container.innerHTML = '<p style="color:white; text-align:center;">No questions found in database.</p>';
                }
            } catch (err) {
                console.error(err);
                container.innerHTML = '<p style="color:white; text-align:center;">Error loading questions.</p>';
            }
        }
        ...
        function renderGame() {
            ...
            // Shuffle the correct and wrong options
            let options = [
                { text: questions[currentQ].correct, className: 'button-yellow', isCorrect: true },
                { text: questions[currentQ].wrong, className: 'button-green', isCorrect: false }
            ];
            options.sort(() => Math.random() - 0.5); // simple shuffle

            contentBox.innerHTML = `
                <button class="back-button" onclick="restartGame()">← Back</button>
                <h1>Guess What: Let Us Try</h1>
                <p class="subtitle">Choose the correct name!</p>
            ...
            ${questions.map((_, i) => `
                ...
                <div class="image-box">
                    <img src="${questions[currentQ].image}" alt="question image" style="max-width:200px; max-height:200px;">
                </div>
                <div class="buttons-container">
                    ${options.map(opt => `
                        <button class="answer-button ${opt.className}" onclick="handleAnswer('${opt.text}')">
                            ${opt.text}
                        </button>
            `).join('')}
                </div>
            ...
            const existingContent = container.querySelector('.content-box');
        ...
        function renderResult() {
            ...
            contentBox.innerHTML = `
                <div class="result-container">
                    ...
                    <div class="result-title">Amazing Work!</div>
        ...
        function handleAnswer(selected) {
            ...
            if (isCorrect) {
                ...
                feedbackEl.textContent = `❌ Oops! It's "${questions[currentQ].correct}"`;
            }
        ...
        function initBackgroundMusic() {
            bgMusic = new Audio('/SMARTY-PLAYGROUND/game/sounds/bg_game4.mp3');
            bgMusic.loop = true;
